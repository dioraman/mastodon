...
class DeliverToDeviceService < BaseService
  ...
  def sign_message_franking!
    @message_franking = Oj.dump(ActivityPub::LinkedDataSignature.new(message_franking).sign!(Account.representative))
  end

  def deliver_to_local!
    target_device = @target_account.devices.find_by!(device_id: @target_device_id)
    target_device.encrypted_messages.create!(from_account: @source_account, from_device_id: @source_device.device_id, type: @type, body: @body, message_franking: @message_franking)
  end
